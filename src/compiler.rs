...
const REGISTER_MAP: [Register; 11] = [
...
pub fn emit<T>(jit: &mut Compiler, data: T) {
    ...
    jit.offset_in_text_section += mem::size_of::<T>() as u32;
}
...
fn emit_validate_instruction_count(jit: &mut Compiler, exclusive: bool, pc: Option<usize>) {
    ...
    if let Some(pc) = pc {
        ...
        emit_ins(jit, RiscVInstruction::cmp(OperandSize::S64, R11, ARGUMENT_REGISTERS[0], None));
    }
...
fn emit_profile_instruction_count(jit: &mut Compiler, target_pc: Option<usize>) {
    match target_pc {
        ...
        None => {
            ...
            emit_ins(jit, RiscVInstruction::alu(OperandSize::S64, 0x01, R11, ARGUMENT_REGISTERS[0], jit.pc as i64, None)); // instruction_meter += target_pc;
        },
    }
}
...
fn emit_profile_instruction_count_finalize(jit: &mut Compiler, store_pc_in_exception: bool) {
    if jit.config.enable_instruction_meter || store_pc_in_exception {
        emit_ins(jit, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, R11, 1, None)); // R11 += 1;
    }
    if jit.config.enable_instruction_meter {
        emit_ins(jit, RiscVInstruction::alu(OperandSize::S64, 0x29, R11, ARGUMENT_REGISTERS[0], 0, None)); // instruction_meter -= pc + 1;
    }
    if store_pc_in_exception {
        emit_ins(jit, RiscVInstruction::load(OperandSize::S64, RBP, R10, X86IndirectAccess::Offset(slot_on_environment_stack(jit, EnvironmentStackSlot::OptRetValPtr))));
        ...
        emit_ins(jit, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, R11, ebpf::ELF_INSN_DUMP_OFFSET as i64 - 1, None));
        emit_ins(jit, RiscVInstruction::store(OperandSize::S64, R11, R10, X86IndirectAccess::Offset((std::mem::size_of::<u64>() * (jit.err_kind_offset + 1)) as i32))); // result.pc = jit.pc + ebpf::ELF_INSN_DUMP_OFFSET;
    }
}
...
pub struct Compiler {
    ...
    offset_in_text_section: u32,
    pc: u32,
    last_instruction_meter_validation_pc: u32,
...
impl Compiler {
    ...
    fn compile<E: UserDefinedError, I: InstructionMeter>(&mut self,
        ...
        while self.pc * ebpf::INSN_SIZE < program.len() {
            ...
            if self.config.enable_instruction_tracing {
                emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, R11, self.pc as i64));
                emit_ins(self, RiscVInstruction::call_immediate(self.relative_to_anchor(ANCHOR_TRACE, 5)));
                emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, R11, 0));
            }
            ...
            match insn.opc {
                _ if insn.dst == STACK_PTR_REG as u8 && self.config.dynamic_stack_frames => {
                    ...
                    match insn.opc {
                        ebpf::SUB64_IMM => emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 5, RBP, insn.imm, Some(stack_ptr_access))),
                        ebpf::ADD64_IMM => emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RBP, insn.imm, Some(stack_ptr_access))),
                        _ => {
                ...
                ebpf::LD_DW_IMM  => {
                    ...
                    if should_sanitize_constant(self, insn.imm) {
                        ...
                        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, dst, insn.imm));
                    }
                },
                ...
                ebpf::LD_B_REG   => {
                    ...
                    emit_ins(self, RiscVInstruction::load(OperandSize::S8, R11, dst, X86IndirectAccess::Offset(0)));
                },
                ebpf::LD_H_REG   => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(src, insn.off as i64, true), 2, AccessType::Load);
                    emit_ins(self, RiscVInstruction::load(OperandSize::S16, R11, dst, X86IndirectAccess::Offset(0)));
                },
                ebpf::LD_W_REG   => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(src, insn.off as i64, true), 4, AccessType::Load);
                    emit_ins(self, RiscVInstruction::load(OperandSize::S32, R11, dst, X86IndirectAccess::Offset(0)));
                },
                ebpf::LD_DW_REG  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(src, insn.off as i64, true), 8, AccessType::Load);
                    emit_ins(self, RiscVInstruction::load(OperandSize::S64, R11, dst, X86IndirectAccess::Offset(0)));
                },

                // BPF_ST class
                ebpf::ST_B_IMM   => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 1, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store_immediate(OperandSize::S8, R11, X86IndirectAccess::Offset(0), insn.imm as i64));
                },
                ebpf::ST_H_IMM   => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 2, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store_immediate(OperandSize::S16, R11, X86IndirectAccess::Offset(0), insn.imm as i64));
                },
                ebpf::ST_W_IMM   => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 4, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store_immediate(OperandSize::S32, R11, X86IndirectAccess::Offset(0), insn.imm as i64));
                },
                ebpf::ST_DW_IMM  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 8, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store_immediate(OperandSize::S64, R11, X86IndirectAccess::Offset(0), insn.imm as i64));
                },

                // BPF_STX class
                ebpf::ST_B_REG  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 1, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store(OperandSize::S8, src, R11, X86IndirectAccess::Offset(0)));
                },
                ebpf::ST_H_REG  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 2, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store(OperandSize::S16, src, R11, X86IndirectAccess::Offset(0)));
                },
                ebpf::ST_W_REG  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 4, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store(OperandSize::S32, src, R11, X86IndirectAccess::Offset(0)));
                },
                ebpf::ST_DW_REG  => {
                    emit_address_translation(self, R11, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 8, AccessType::Store);
                    emit_ins(self, RiscVInstruction::store(OperandSize::S64, src, R11, X86IndirectAccess::Offset(0)));
                },
                ...
                ebpf::MOV32_IMM  => {
                    if should_sanitize_constant(self, insn.imm) {
                        ...
                        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S32, dst, insn.imm));
                    }
                }
                ...
                ebpf::JA         => {
                    ...
                    emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, R11, target_pc as i64));
                ...
                ebpf::CALL_IMM   => {
                    ...
                    if syscalls {
                        if let Some(syscall) = executable.get_syscall_registry().lookup_syscall(insn.imm as u32) {
                            ...
                            emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, R11, syscall.function as *const u8 as i64));
                ...
                ebpf::EXIT      => {
                    ...
                    emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, REGISTER_MAP[FRAME_PTR_REG], call_depth_access));
                    ...
                    emit_ins(self, RiscVInstruction::store(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], RBP, call_depth_access));
    ...
    fn generate_prologue<E: UserDefinedError, I: InstructionMeter>(&mut self, executable: &Executable<E, I>) -> Result<(), EbpfError<E>> {
        ...
        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], 0));
        ...
        if self.config.dynamic_stack_frames {
            ...
            emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], MM_STACK_START as i64 + self.config.stack_frame_size as i64));
            // Push BpfFramePtr
            emit_ins(self, RiscVInstruction::push(REGISTER_MAP[FRAME_PTR_REG], None));
            // When using static frames BpfStackPtr is not used
            emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, RBP, 0));
            emit_ins(self, RiscVInstruction::push(RBP, None));
        }

        // Save pointer to optional typed return value
        emit_ins(self, RiscVInstruction::push(ARGUMENT_REGISTERS[0], None));
        ...
        emit_ins(self, RiscVInstruction::push(ARGUMENT_REGISTERS[3], None));

        // Initialize stop watch
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x31, R11, R11, 0, None)); // R11 ^= R11;
        emit_ins(self, RiscVInstruction::push(R11, None));
        emit_ins(self, RiscVInstruction::push(R11, None));

        // Initialize frame pointer
        emit_ins(self, RiscVInstruction::mov(OperandSize::S64, RSP, RBP));
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RBP, 8 * (EnvironmentStackSlot::SlotCount as i64 - 1 + self.environment_stack_key as i64), None));

        // Save ProgramEnvironment
        emit_ins(self, RiscVInstruction::lea(OperandSize::S64, ARGUMENT_REGISTERS[2], R10, Some(X86IndirectAccess::Offset(-self.program_argument_key))));

        // Zero BPF registers
        for reg in REGISTER_MAP.iter() {
            if *reg != REGISTER_MAP[1] && *reg != REGISTER_MAP[FRAME_PTR_REG] {
                emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, *reg, 0));
            }
        }
        ...
        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, R11, entry as i64));
    ...
    fn generate_subroutines<E: UserDefinedError, I: InstructionMeter>(&mut self) -> Result<(), EbpfError<E>> {
        ...
        emit_ins(self, RiscVInstruction::lea(OperandSize::S64, RBP, RSP, Some(X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::LastSavedRegister)))));
        // Restore registers
        ...
        if self.config.enable_instruction_tracing {
            ...
            emit_ins(self, RiscVInstruction::mov(OperandSize::S64, RSP, REGISTER_MAP[0]));
            emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RSP, - 8 * 3, None)); // RSP -= 8 * 3;
            ...
            emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RSP, 8 * (REGISTER_MAP.len() - 1) as i64, None)); // RSP += 8 * (REGISTER_MAP.len() - 1);
            emit_ins(self, RiscVInstruction::pop(R11));
            emit_ins(self, RiscVInstruction::return_near());
        }
        ...
        emit_ins(self, RiscVInstruction::mov(OperandSize::S64, ARGUMENT_REGISTERS[0], R11)); // R11 = instruction_meter;
        ...
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RSP, R11, X86IndirectAccess::OffsetIndexShift(-16, RSP, 0))); // R11 = RSP[-16];
        ...
        emit_validate_instruction_count(self, false, None);
        emit_profile_instruction_count_finalize(self, false);
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, R10, X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::OptRetValPtr))));
        ...
        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, REGISTER_MAP[0], 0));
        emit_ins(self, RiscVInstruction::jump_immediate(self.relative_to_anchor(ANCHOR_EPILOGUE, 5)));

        // Routine for syscall
        self.set_anchor(ANCHOR_SYSCALL);
        emit_ins(self, RiscVInstruction::push(R11, None)); // Padding for stack alignment
        if self.config.enable_instruction_meter {
            // RDI = *PrevInsnMeter - RDI;
            emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x2B, ARGUMENT_REGISTERS[0], RBP, 0, Some(X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::PrevInsnMeter))))); // RDI -= *PrevInsnMeter;
        ...
        if self.config.enable_instruction_meter {
            ...
            emit_ins(self, RiscVInstruction::store(OperandSize::S64, ARGUMENT_REGISTERS[0], RBP, X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::PrevInsnMeter))));
        }
        ...
        emit_ins(self, RiscVInstruction::pop(R11));
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, R11, X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::OptRetValPtr))));
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, R11, REGISTER_MAP[0], X86IndirectAccess::Offset(8)));
        emit_ins(self, RiscVInstruction::return_near());

        // Routine for prologue of emit_bpf_call()
        self.set_anchor(ANCHOR_BPF_CALL_PROLOGUE);
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 5, RSP, 8 * (SCRATCH_REGS + 1) as i64, None)); // alloca
        emit_ins(self, RiscVInstruction::store(OperandSize::S64, R11, RSP, X86IndirectAccess::OffsetIndexShift(0, RSP, 0))); // Save original R11
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RSP, R11, X86IndirectAccess::OffsetIndexShift(8 * (SCRATCH_REGS + 1) as i32, RSP, 0))); // Load return address
        for (i, reg) in REGISTER_MAP.iter().skip(FIRST_SCRATCH_REG).take(SCRATCH_REGS).enumerate() {
            emit_ins(self, RiscVInstruction::store(OperandSize::S64, *reg, RSP, X86IndirectAccess::OffsetIndexShift(8 * (SCRATCH_REGS - i + 1) as i32, RSP, 0))); // Push SCRATCH_REG
        }
        // Push the caller's frame pointer. The code to restore it is emitted at the end of emit_bpf_call().
        emit_ins(self, RiscVInstruction::store(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], RSP, X86IndirectAccess::OffsetIndexShift(8, RSP, 0)));
        emit_ins(self, RiscVInstruction::xchg(OperandSize::S64, R11, RSP, Some(X86IndirectAccess::OffsetIndexShift(0, RSP, 0)))); // Push return address and restore original R11

        // Increase CallDepth
        let call_depth_access = X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::CallDepth));
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RBP, 1, Some(call_depth_access)));
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, REGISTER_MAP[FRAME_PTR_REG], call_depth_access));
        ...
        if self.config.dynamic_stack_frames {
            ...
            let stack_ptr_access = X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::BpfStackPtr));
            emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, REGISTER_MAP[FRAME_PTR_REG], stack_ptr_access));
            emit_ins(self, RiscVInstruction::store(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], RBP, frame_ptr_access));
        } else {
            ...
            emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RBP, stack_frame_size, Some(frame_ptr_access))); // frame_ptr += stack_frame_size;
            emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, REGISTER_MAP[FRAME_PTR_REG], frame_ptr_access)); // Load BpfFramePtr
        }
        emit_ins(self, RiscVInstruction::return_near());
        ...
        emit_ins(self, RiscVInstruction::load_immediate(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], self.program_vm_addr as i64));
        emit_ins(self, RiscVInstruction::cmp(OperandSize::S64, REGISTER_MAP[FRAME_PTR_REG], REGISTER_MAP[0], None));
        ...
        emit_ins(self, RiscVInstruction::mov(OperandSize::S64, REGISTER_MAP[0], R11));
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0xc1, 5, R11, shift_amount as i64, None));
        // Save BPF target pc for potential ANCHOR_CALLX_UNSUPPORTED_INSTRUCTION
        emit_ins(self, RiscVInstruction::store(OperandSize::S64, R11, RSP, X86IndirectAccess::OffsetIndexShift(-8, RSP, 0))); // RSP[-8] = R11;
        ...
        emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, REGISTER_MAP[FRAME_PTR_REG], X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::BpfFramePtr))));
        emit_ins(self, RiscVInstruction::return_near());
        ...
        emit_ins(self, RiscVInstruction::cmp(OperandSize::S64, R11, REGISTER_MAP[0], Some(X86IndirectAccess::Offset(8)))); // Look up and compare against value at next index
        emit_ins(self, RiscVInstruction::conditional_jump_immediate(0x86, self.relative_to_anchor(ANCHOR_TRANSLATE_PC_LOOP, 6))); // Continue while *REGISTER_MAP[0] <= R11
        emit_ins(self, RiscVInstruction::mov(OperandSize::S64, REGISTER_MAP[0], R11)); // R11 = REGISTER_MAP[0];
        ...
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x29, REGISTER_MAP[0], R11, 0, None)); // R11 -= REGISTER_MAP[0];
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0xc1, 5, R11, 3, None)); // R11 >>= 3;
        emit_ins(self, RiscVInstruction::pop(REGISTER_MAP[0])); // Restore REGISTER_MAP[0]
        emit_ins(self, RiscVInstruction::return_near());

        self.set_anchor(ANCHOR_MEMORY_ACCESS_VIOLATION);
        emit_ins(self, RiscVInstruction::alu(OperandSize::S64, 0x81, 0, RSP, 8, None));
        emit_ins(self, RiscVInstruction::pop(R11)); // Put callers PC in R11
        ...
        for (access_type, len) in &[
            ...
            emit_ins(self, RiscVInstruction::load(OperandSize::S64, RBP, R11, X86IndirectAccess::Offset(slot_on_environment_stack(self, EnvironmentStackSlot::OptRetValPtr))));
            emit_ins(self, RiscVInstruction::load(OperandSize::S64, R11, R11, X86IndirectAccess::Offset(8)));
